...
def snapshot():
    try:
        ...
        load_env()
        t, v = ecos_spot()
        return {"rate": v, "date": f"{t[:4]}-{t[4:6]}-{t[6:]}", "source": "ecos",
...
def main():
    ...
    if data["source"] == "ecos":
        print(f"[fx] ✅ ECOS {data['date']} · {data['rate']:,.2f} KRW/USD → {out}")
    else:
        print(f"[fx] ⚠️  폴백 {data['rate']:,.2f} 사용 (사유: {data.get('error', '')[:80]}) → {out}")
